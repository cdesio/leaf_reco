...
def run_leaf_mask_inv(in_folder, dist, out_folder):
    ...
    f_tot = [np.int(regex.findall(i)[0]) for i in os.listdir(in_folder) if "mask" not in i and str(i).startswith("File")]
    ...
    for nb in sorted(fbad):
        #print(nb)
        os.system("python ~/physics/leaf_reco/Leaf_mask_inv.py {} {} {} {}".format(in_folder, nb, dist, out_folder))
    print("masks created!")
